...
for i in "${!SESSION_COUNTS[@]}"; do
    ...
    # After starting 2 jobs (filling both GPUs), wait for the first to finish before starting the next
    if [ $(((i + 1) % 2)) -eq 0 ] && [ $i -lt $((${#SESSION_COUNTS[@]} - 1)) ]; then
        echo "Both GPUs occupied, waiting for one to finish..."
        wait -n
        EXIT_CODE=$?
        TRAIN_END=$(date +%s)
        ...
        echo "Train complete: $(date +%Y%m%d_%H%M%S), Duration: ${TRAIN_TIME}s" >> ${TIMING_LOG}
        echo "GPU freed, continuing..."
